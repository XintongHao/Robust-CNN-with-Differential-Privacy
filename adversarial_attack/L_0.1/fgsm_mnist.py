...
def model(x, logits=False, training=False):
    with tf.variable_scope('conv0'):
        ...
        z = slim.conv2d(x, 32, [5, 5])
        z = slim.max_pool2d(z, [2, 2])
#        z = tf.layers.conv2d(x, filters=32, kernel_size=[3, 3],
#                             padding='same', activation=tf.nn.relu)
#        z = tf.layers.conv2d(x, filters=32, kernel_size=[1, 1],
#                          padding='same', activation=tf.nn.relu)
#
    ...
    noise = tf.random_normal(tf.shape(z), mean=0, stddev=1)
...
#        z = tf.reshape(z, [-1, np.prod(shape[1:])])
    ...
    with tf.variable_scope('mlp'):
        ...
        net = slim.dropout(net, is_training=training)  # 0.5 by default
#        z = tf.layers.dense(z, units=128, activation=tf.nn.relu)
#        z = tf.layers.dropout(z, rate=0.25, training=training)

#    logits_ = tf.layers.dense(z, units=10, name='logits')
#    outputs = slim.fully_connected(net, 10, activation_fn=None, normalizer_fn=None, scope='fco')
    logits_ = tf.layers.dense(net, units=10, name='logits')
...
with tf.variable_scope('model'):
    ...
    with tf.variable_scope('loss'):
        xent = tf.nn.softmax_cross_entropy_with_logits(labels=env.y,
                                                       logits=logits)
#        env.loss = slim.losses.softmax_cross_entropy(env.y, env.ybar, scope='loss')
